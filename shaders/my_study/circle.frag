	...
	float circle(vec2 st, float pct1, float pct2)
{
	...
	{
		// 返回点到圆形边缘的距离
		// float distance = distance(pos, origin);  // sqrt(pow(pos.x - origin.x, 2.0) + pow(pos.y - origin.y, 2.0));
		float sdf = distance(pos, origin) - radius;

        return abs(sdf / radius);
	}
float plane = 0.1;  // 地平线
float speed = 0.05;  // 太阳升起速度
void main() {
    ...
    vec3 midlle_left_to_right = vec3((smoothstep(0., 0.3, st.y) - smoothstep(0.3, 0.45, st.y)) * (1.0 - st.x));

    vec3 colorA = vec3(0.149,0.141,0.912);
    vec3 colorB = vec3(0.7647, 0.9725, 0.0196);
...
//     color = mix(color,vec3(0.0,0.0,1.0),plot(st,pct.b));
   ...
   //  float is_cir = circle(st, pct1, pct2);
    ...
    color = mix(color, vec3(0.0196, 0.1137, 0.9529) * 10., is_cir);
